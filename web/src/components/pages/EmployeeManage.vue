  ...
  <div class="content-warp">
    ...
    <div class="table-box">
      ...
      <el-table
        ...
        <el-table-column prop="login_time" label="最近登录时间" min-width="120" />
...
export default {
  ...
  methods: {
    ...
    saveAdd() {
      const {nickName, pwd, auth} = this.add
      addEmployee({
        nickName, pwd, auth: 'employee'
      }).then(res => {
        if(res.data.success) {
          this.api_getDate()
          this.showAdd = false
    ...
    saveEdit() {  // 保存编辑
      ...
      editEmployee({
        _id, nickName, pwd, auth: 'employee'
      }).then(res => {
        if(res.data.success) {
          this.api_getDate()
          this.showEdit = false
